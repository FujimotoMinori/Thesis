...
The unitarity bound can be theoretically calculated in each willson coefficients.
...
The likelihood function is defined same as the standard model fitting described in Chapter{}. According to the Wild theorem~\ref{}, likelihood coefficient distribution asymptotically converges to a $\chi^2$ distribution. 
Therefore the $\chi^2$ distribution can be used to approximately estimate limits for $f/\Lambda^4$. For a 95\% level of confidence, the limits, which is the boundaries of the corresponding interval of the $\chi^2$ distribution is derived.
The limit on $f/\Rambda^4$ is directory obtained from the interval of the likelihood function. The interference term is counted into the aQGC signal with the quadratic term by interpreting as $\mu_{QUAD} = \mu_{INT}^2$. 
