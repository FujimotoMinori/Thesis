...
The aQGC signal is extacted with a profile-likelihood fit same as what described in section~\ref{sec:likelihood}, where the signal referring the aQGC signal sample, and the standard model EW VV+jj signal sample is included as background with other standard model background samples like V+jets and $t\bar{t}$.
...
Confidence intervals are calculated with asympotoic fomulae using Wilks' theorem~\ref{}, assuming that the profile likelihood test statistic is $\chi^2$ distributed~\cite{}.
...
The SM EW VV+jj signal samples are added to the background as free floated normalization factor, and all the analysis regions are correlated.
...
The log-likelihood (-2 ln $\lambda$, $\lambda$ is a likelihood ratio) distributions with respect to the signal strength of aQGC signal, $\mu_{\mathrm{aQGC}}$, is shown in figure~\ref{fig:ProfileLL} for FT0 operator, for each clipping energy of 1500~GeV, 2000~GeV, 3000~GeV, $\infty$.
...
\begin{figure}[ht]
    ...
    \includegraphics[width=0.38\textwidth]{figures/aQGC/profileFT01500}
        ...
        \caption{The observed log-likelihood curves of FT0 Wilson coeffient where the clipping energy is 1.5~TeV (top left), 2.0~TeV (top right), 3.0~TeV (bottom left), $\infty$ (bottom right).}
        \label{fig:ProfileLL}
\end{figure}
...
For high clipping values, the results exclude the signal strength which are already exluded by the theoretical unitarity bound.
Table~\ref{tab:aQGClimits} shows the uniterized and ununiterized expected and observed limits from figure~\ref{fig:aQGClimits}, where uniterized limit is the intersection point of the thoretical unitality bound and experimental limit line, whilde ununiterized limit is the limit with no clipping energy.
\begin{figure}[ht]
    \centering
