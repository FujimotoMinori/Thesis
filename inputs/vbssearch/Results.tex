...
\label{sec:confirm}
The validity of the fitting setup is confirmed by checking the background modeling after fitting in the CRs.
This validation procedure is performed by not looking at the observed data in the signal-sensitive regions.
In the following ’left-side’bins of the SR discriminant distribution is refered to as the bins in the SR that contains 25~\% of the signal sample.
The result of the fitting using $m^{tag}_{jj}$ in CRs and the left-side bins of the RNN score in SRs are applied to the $m^{tag}_{jj}$ and the RNN score distributions. 
Obtained distributions in the CRs are shown in figure~\ref{fig:postCheck}.
%Control region validation plots
\begin{figure}[]
    \centering
    %0lep
    \includegraphics[width=0.45\textwidth]{figures/validation/}
    \includegraphics[width=0.45\textwidth]{figures/validation/}
    %1lep
    \includegraphics[width=0.45\textwidth]{figures/validation/}
    \includegraphics[width=0.45\textwidth]{figures/validation/}
