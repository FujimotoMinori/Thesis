...
The event selection has been optimized for the SM VBS signals against the background processes.
...
%\subsection{Triggrs}
%Trigger selection is applied in order to select events that are interesting. Single 
\subsection{Leptons}
The electrons and muons used in this analysis are selected at first.
"Tight" or "Loose" selection is applied, as the table~\ref{} shows. 
Both electrons and muons are required to have a transvers impact aparameter significance relative to the beam line ($|d_0/|sigma_{d_0}|$) cut.
They are also required to satisfy isolation citeria based on both track and calorimeter isolation variables by requiring several isolation critaria.

\subsection{small-R jet}
Small-R jets are used to reconstruct the hadronically decaying boson.
...
Two same flavor loose-leptons with the leading (sub leading) lepton satisfying $p_T$ > 28~GeV ( $p_T$ > 20~GeV) and an invariant mass of the lepton system ( m$_{ll}$ ) close to the Z boson mass are requested. Figure~\ref{fig:2lepLeptons} shows nice peak of the invariant mass of the two selected leptons around the Z boson mass. Depending on the lepton pair (ee, $\mu\mu$) different mass
